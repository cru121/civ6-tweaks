...
-- I2 fix (save/reload culture stacking): this effect MUST use ADD_RELIGIOUS_BELIEF_YIELD,
-- not ADD_PLAYER_BELIEF_YIELD. Delivered through a re-attaching collection
-- (MODIFIER_ALL_PLAYERS_ATTACH_MODIFIER), the PLAYER variant re-applies and STACKS on
-- every save/reload (culture grows without bound). The RELIGIOUS variant writes to the
-- religion idempotently and does not stack -- matching base-game Pilgrimage/World Church,
-- which use the same attach with the RELIGIOUS variant.
...
VALUES	('MODIFIER_TM_FEATURE_KAILASH_ATTACH_PLAYERS',	'MODIFIER_ALL_PLAYERS_ATTACH_MODIFIER',				'REQSET_TM_MAP_HAS_FEATURE_KAILASH',	'REQSET_TM_PLAYER_HAS_FEATURE_KAILASH'	),
		('MODIFIER_TM_FEATURE_KAILASH_CULTURE_CITIES',	'MODIFIER_PLAYER_RELIGION_ADD_RELIGIOUS_BELIEF_YIELD',	NULL,									NULL									);
